...
%   figure(); imshow(mask)  %%% FIGURE
    ...
    minx=max(min(x1),min(x2)); % the one more on the right, then max of mins
    mask(:,1:minx)=0;   
    ...
    % CLOSE THE IMT REGION CREATING THE COLUMNS IN THE LEFT AND RIGHT LIMITS
    limLeft=find(mask(:,(minx+1))==1)    
%      if isempty(limLeft) 
%          return
%      end    
    mask(limLeft(1):limLeft(end),(minx+1))=1;    
    limRight=find(mask(:,(maxx-1))==1)
    mask(limRight(1):limRight(end),(maxx-1))=1;
